...
const FIELDS: Field<LoginForm>[] = [
	...
	{ autoComplete: "password", inputType: "password", labelText: "Password", name: "password", validation: { required: "Please enter a password" } },
];
	...
	return (
		<main className="flex justify-center items-center shrink-0 grow">
			<form onSubmit={handleSubmit(onSubmit)} className="relative flex flex-col items-center shadow [&>label]:mb-6 p-8 border border-blue-light rounded-lg [&>label]:w-full md:w-96">
				...
				{FIELDS.map(({ autoComplete, inputType, name, labelText, validation }) => {
					return (
				...
				{errors.root && <span className="input-error bottom-1/2 translate-y-9.5 text-center">Incorrect email or password</span>}
